...
func TestServer_ListFeeRecipientByPubkey(t *testing.T) {
	...
	require.NoError(t, err)
	recipient0 := common.MustParseAddress(standardAPIFeeRecipient0)
	recipient1 := common.MustParseAddress(standardAPIFeeRecipient1)
...
func TestServer_FeeRecipientByPubkey(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args:             standardAPIFeeRecipient0,
			proposerSettings: nil,
			want: &want{
				valQRLAddress: standardAPIFeeRecipient0,
			},
			wantErr: false,
			beaconReturn: &beaconResp{
				resp:  nil,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			qrlAddr := common.MustParseAddress(tt.args)
...
func TestServer_DeleteFeeRecipientByPubkey(t *testing.T) {
	...
	require.NoError(t, err)
	recipient0 := common.MustParseAddress(standardAPIFeeRecipient2)
	recipient1 := common.MustParseAddress(standardAPIFeeRecipient0)
	type want struct {
		QRLAddress string
	}
	tests := []struct {
