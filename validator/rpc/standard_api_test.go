...
func TestServer_ListFeeRecipientByPubkey(t *testing.T) {
	...
	require.NoError(t, err)
	recipient0, err := common.NewAddressFromString(standardAPIFeeRecipient0)
	require.NoError(t, err)
	recipient1, err := common.NewAddressFromString(standardAPIFeeRecipient1)
	require.NoError(t, err)
...
func TestServer_DeleteFeeRecipientByPubkey(t *testing.T) {
	...
	require.NoError(t, err)
	recipient0, err := common.NewAddressFromString(standardAPIFeeRecipient2)
	require.NoError(t, err)
	recipient1, err := common.NewAddressFromString(standardAPIFeeRecipient0)
	require.NoError(t, err)
	type want struct {
		QRLAddress string
	}
