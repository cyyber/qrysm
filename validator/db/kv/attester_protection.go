		...
		return sourceEpochsBucket.ForEach(func(sourceBytes, targetEpochsList []byte) error {
			...
			for _, targetEpoch := range targetEpochs {
				...
				if len(signingRoot) != 0 {
					record.SigningRoot = make([]byte, field_params.RootLength)
...
func (s *Store) SigningRootAtTargetEpoch(ctx context.Context, pubKey [field_params.MLDSA87PubkeyLength]byte, target primitives.Epoch) ([]byte, error) {
