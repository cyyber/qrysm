...
func (s *Store) ProposalHistoryForSlot(ctx context.Context, publicKey [field_params.MLDSA87PubkeyLength]byte, slot primitives.Slot) ([32]byte, bool, bool, error) {
...
func (s *Store) ProposalHistoryForPubKey(ctx context.Context, publicKey [field_params.MLDSA87PubkeyLength]byte) ([]*Proposal, error) {
	...
	err := s.view(func(tx *bolt.Tx) error {
		...
		return valBucket.ForEach(func(slotKey, signingRootBytes []byte) error {
			...
			sr := make([]byte, field_params.RootLength)
