...
func UnpackDepositLogData(data []byte) (pubkey, withdrawalRecipient, amount, randaoCommitment, signature, index []byte, err error) {
	...
	if len(unpackedLogs) != 6 {
		return nil, nil, nil, nil, nil, nil, errors.Errorf("unexpected number of deposit event fields: %d", len(unpackedLogs))
	}
