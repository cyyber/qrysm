...
func Test_Proposer_Setting_Cloning(t *testing.T) {
	...
	feeRecipient0, err := common.NewAddressFromString(testFeeRecipient0)
	require.NoError(t, err)
	feeRecipient1, err := common.NewAddressFromString(testFeeRecipient1)
	require.NoError(t, err)
	...
	t.Run("Happy Path Cloning", func(t *testing.T) {
		...
		newFeeRecipient, err := common.NewAddressFromString(testFeeRecipient2)
		require.NoError(t, err)
