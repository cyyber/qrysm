...
func TestConfigureExecutionSetting(t *testing.T) {
	...
	err = configureExecutionSetting(cliCtx)
	require.NoError(t, err)
	recipient0, err := common.NewAddressFromString(configTestSuggestedFeeRecipient0)
	require.NoError(t, err)
	...
	err = configureExecutionSetting(cliCtx)
	require.NoError(t, err)
	recipient1, err := common.NewAddressFromString(configTestSuggestedFeeRecipient1)
	require.NoError(t, err)
