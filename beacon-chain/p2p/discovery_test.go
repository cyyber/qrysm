	...
	tests := []struct {
	...
	}{
		{
			...
			svcBuilder: func(t *testing.T) *Service {
				...
				_ = s.updateSubnetRecordWithMetadata([]byte{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, []byte{})
				return s
			},
			postValidation: func(t *testing.T, s *Service) {
		...
		},
		{
			...
			svcBuilder: func(t *testing.T) *Service {
				...
				s.dv5Listener = listener
				s.metaData = wrapper.WrappedMetadataV1(new(qrysmpb.MetaDataV1))
				_ = s.updateSubnetRecordWithMetadata([]byte{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01}, []byte{})
