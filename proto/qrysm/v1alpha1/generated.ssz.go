...
func (v *ValidatorRegistrationV1) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	if size := len(v.FeeRecipient); size != 48 {
...
func (v *ValidatorRegistrationV1) UnmarshalSSZ(buf []byte) error {
	...
	if cap(v.FeeRecipient) == 0 {
		v.FeeRecipient = make([]byte, 0, len(buf[0:48]))
	}
	v.FeeRecipient = append(v.FeeRecipient, buf[0:48]...)

	// Field (1) 'GasLimit'
	v.GasLimit = ssz.UnmarshallUint64(buf[48:56])

	// Field (2) 'Timestamp'
	v.Timestamp = ssz.UnmarshallUint64(buf[56:64])

	// Field (3) 'Pubkey'
	if cap(v.Pubkey) == 0 {
		v.Pubkey = make([]byte, 0, len(buf[64:2656]))
	}
	v.Pubkey = append(v.Pubkey, buf[64:2656]...)
...
func (s *SignedValidatorRegistrationV1) UnmarshalSSZ(buf []byte) error {
	...
	if err = s.Message.UnmarshalSSZ(buf[0:2656]); err != nil {
		return err
	}

	// Field (1) 'Signature'
	if cap(s.Signature) == 0 {
		s.Signature = make([]byte, 0, len(buf[2656:7283]))
	}
	s.Signature = append(s.Signature, buf[2656:7283]...)
...
func (b *BeaconStateZond) UnmarshalSSZ(buf []byte) error {
	...
	{
		buf = tail[o11:o12]
		num, err := ssz.DivideInt2(len(buf), 2697, 1099511627776)
		if err != nil {
			return err
		}
		...
		for ii := 0; ii < num; ii++ {
			...
			if err = b.Validators[ii].UnmarshalSSZ(buf[ii*2697 : (ii+1)*2697]); err != nil {
				return err
			}
		}
	}
