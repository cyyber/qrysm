...
func TestBeaconStateZondToProto(t *testing.T) {
	source, err := util.NewBeaconStateZond(util.FillRootsNaturalOptZond, func(state *qrysmpb.BeaconStateZond) error {
		...
		state.Validators = []*qrysmpb.Validator{{
			...
			WithdrawalRecipient:        bytesutil.PadTo([]byte("withdrawalcredentials"), 64),
			EffectiveBalance:           9,
			Slashed:                    true,
	...
	assert.DeepEqual(t, bytesutil.PadTo([]byte("withdrawalcredentials"), fieldparams.WithdrawalRecipientLength), resultValidator.WithdrawalRecipient)
