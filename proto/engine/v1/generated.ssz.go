...
func (e *ExecutionPayloadZond) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	if size := len(e.FeeRecipient); size != 48 {
		err = ssz.ErrBytesLengthFn("--.FeeRecipient", size, 48)
		return
	}
...
func (e *ExecutionPayloadZond) UnmarshalSSZ(buf []byte) error {
	...
	if cap(e.FeeRecipient) == 0 {
		e.FeeRecipient = make([]byte, 0, len(buf[32:80]))
	}
	e.FeeRecipient = append(e.FeeRecipient, buf[32:80]...)

	// Field (2) 'StateRoot'
	if cap(e.StateRoot) == 0 {
		e.StateRoot = make([]byte, 0, len(buf[80:112]))
	}
	e.StateRoot = append(e.StateRoot, buf[80:112]...)

	// Field (3) 'ReceiptsRoot'
	if cap(e.ReceiptsRoot) == 0 {
		e.ReceiptsRoot = make([]byte, 0, len(buf[112:144]))
	}
	e.ReceiptsRoot = append(e.ReceiptsRoot, buf[112:144]...)

	// Field (4) 'LogsBloom'
	if cap(e.LogsBloom) == 0 {
		e.LogsBloom = make([]byte, 0, len(buf[144:400]))
	}
	e.LogsBloom = append(e.LogsBloom, buf[144:400]...)

	// Field (5) 'PrevRandao'
	if cap(e.PrevRandao) == 0 {
		e.PrevRandao = make([]byte, 0, len(buf[400:432]))
	}
	e.PrevRandao = append(e.PrevRandao, buf[400:432]...)

	// Field (6) 'BlockNumber'
	e.BlockNumber = ssz.UnmarshallUint64(buf[432:440])

	// Field (7) 'GasLimit'
	e.GasLimit = ssz.UnmarshallUint64(buf[440:448])

	// Field (8) 'GasUsed'
	e.GasUsed = ssz.UnmarshallUint64(buf[448:456])

	// Field (9) 'Timestamp'
	e.Timestamp = ssz.UnmarshallUint64(buf[456:464])

	// Offset (10) 'ExtraData'
	if o10 = ssz.ReadOffset(buf[464:468]); o10 > size {
	...
	// Offset (13) 'Transactions'
	if o13 = ssz.ReadOffset(buf[532:536]); o13 > size || o10 > o13 {
		return ssz.ErrOffset
	}

	// Offset (14) 'Withdrawals'
	if o14 = ssz.ReadOffset(buf[536:540]); o14 > size || o13 > o14 {
		return ssz.ErrOffset
	}
	...
	// Field (14) 'Withdrawals'
	{
		...
		for ii := 0; ii < num; ii++ {
			...
			if err = e.Withdrawals[ii].UnmarshalSSZ(buf[ii*72 : (ii+1)*72]); err != nil {
				return err
			}
		}
	}
...
func (e *ExecutionPayloadHeaderZond) UnmarshalSSZ(buf []byte) error {
	...
	if cap(e.TransactionsRoot) == 0 {
		e.TransactionsRoot = make([]byte, 0, len(buf[532:564]))
	}
	e.TransactionsRoot = append(e.TransactionsRoot, buf[532:564]...)

	// Field (14) 'WithdrawalsRoot'
	if cap(e.WithdrawalsRoot) == 0 {
		e.WithdrawalsRoot = make([]byte, 0, len(buf[564:596]))
	}
	e.WithdrawalsRoot = append(e.WithdrawalsRoot, buf[564:596]...)
...
func (w *Withdrawal) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Field (2) 'Address'
	if size := len(w.Address); size != 48 {
		err = ssz.ErrBytesLengthFn("--.Address", size, 48)
		return
	}
...
func (w *Withdrawal) UnmarshalSSZ(buf []byte) error {
	...
	// Field (2) 'Address'
	if cap(w.Address) == 0 {
		w.Address = make([]byte, 0, len(buf[16:64]))
	}
	w.Address = append(w.Address, buf[16:64]...)

	// Field (3) 'Amount'
	w.Amount = ssz.UnmarshallUint64(buf[64:72])
