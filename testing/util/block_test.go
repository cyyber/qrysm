...
func TestGeneratedOperations_DeterministicSignatures(t *testing.T) {
	...
	proposerSlashing, err := GenerateProposerSlashingForValidator(beaconState, privKeys[validatorIndex], validatorIndex)
	require.NoError(t, err)
	proposerSlashingAgain, err := GenerateProposerSlashingForValidator(beaconState, privKeys[validatorIndex], validatorIndex)
	require.NoError(t, err)
	...
	attesterSlashing, err := GenerateAttesterSlashingForValidator(beaconState, privKeys[validatorIndex], validatorIndex)
	require.NoError(t, err)
	attesterSlashingAgain, err := GenerateAttesterSlashingForValidator(beaconState, privKeys[validatorIndex], validatorIndex)
	require.NoError(t, err)
	...
	exit, err := GenerateVoluntaryExits(beaconState, privKeys[0], 0)
	require.NoError(t, err)
	exitAgain, err := GenerateVoluntaryExits(beaconState, privKeys[0], 0)
	require.NoError(t, err)
...
func TestGenerateSyncAggregate_DeterministicSignatures(t *testing.T) {
	...
	aggregate, err := generateSyncAggregate(beaconState, privKeys, parentRoot)
	require.NoError(t, err)
	require.NotEqual(t, 0, len(aggregate.SyncCommitteeSignatures))

	aggregateAgain, err := generateSyncAggregate(beaconState, privKeys, parentRoot)
	require.NoError(t, err)
